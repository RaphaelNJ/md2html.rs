...
Ce programme est un convertisseur Markdown vers HTML développé en Rust. Il vous permet de convertir des fichiers Markdown au format HTML en utilisant des expressions régulières. Le convertisseur est équipé d'un petit serveur et d'un "watcher", un composant de l'API Linux qui permet d'être notifié lors de la modification d'un fichier, offrant ainsi un aperçu en temps réel du rendu HTML.

Veuillez noter que ce programme a été exclusivement testé sur un environnement Linux. Sa compatibilité avec Windows ou iOS n'a pas été vérifiée.
...
- Système d'exploitation Linux (pour une fonctionnalité optimale)
